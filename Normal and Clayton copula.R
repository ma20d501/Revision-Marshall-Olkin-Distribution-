...
power<- rep()
alpha=c(0.02,0.2,0.1)  # <- 0.02
for(s in 1:3){
  sam=  c(25,50,75,100)
  for(d in 1:4){
    n<-sam[d]
    # Generate normal copula samples with dimensionality 2 and 100 samples
    normal <- rCopula(n, normalCopula(0.5, dim = 2))
    x<- normal[,1]
    y<- normal[,2] 
    ## test statistics 
  ...
  power[s]= mean(1* power[power!='NaN'])  
}
power
...
for(d in 1:4){
  n<-sam[d]
  # Generate clayton copula samples with dimensionality 2 and 100 samples
  clayton <- rCopula(n, claytonCopula(2, dim = 2))
  x<- clayton[,1]
  y<- clayton[,2] 
  ## test statistics 
  MOC.Test<-function(x,y){ # here X is an array
